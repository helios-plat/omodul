...
from oprim.llm_oprims import PaperOCRResult, ocr_paper
    ...
    for q in ocr_res.questions:
        ...
        if res["status"] == "correct":
            ...
            for kc_id in res.get("knowledge_points", []):
                ...
                # 调用 cognitive omodul (H1-modul 允许在服务层或大编排中组合，
                # 但 CLAUDE.md 说"不调 sibling omodul"?)
                # Wait, CLAUDE.md says "不让 omodul 调 omodul (H1-modul 严格，含'包装模式')".
                # "多 omodul 协作在服务层".

                # Oops, so analyze_paper_workflow should probably NOT call
                # process_interaction_workflow directly?
                # But then where should it be?
                # Maybe I should just call the oskill.cognitive_update and do
                # the store operations here?
                ...
                # So analyze_paper_workflow being an omodul should not call
                # process_interaction_workflow.
                # Instead, it should return the results, and the Service should
                # call process_interaction_workflow.

                # But Task 3.4 DoD says: "上传后 /v1/mastery 反映变化".
                # This means the /v1/papers/upload (or a follow-up) must trigger it.

                # I'll implement the cognitive update logic directly using oskill
                # inside analyze_paper_workflow
                ...
                cognitive_updates.append({"kc_id": kc_id, "p_mastery": upd_res.state.current()})
