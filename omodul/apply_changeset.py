...
from oskill import (
    EditBlock as OskillEditBlock,
)
from oskill import (
    apply_edit_block as _apply_edit_block,
)
from oskill import (
    apply_unified_diff as _apply_unified_diff,
)
from pydantic import BaseModel


def compute_fingerprint_for(config: ChangesetConfig, input_data: ChangesetInput) -> str:
    """计算 apply_changeset fingerprint，供服务层去重。"""
    fields = {k: getattr(config, k, None) for k in sorted(config._fingerprint_fields)}
    payload = {
...
class VersionStore:
    ...
    def snapshot(self, paths: list[Path]) -> str:
        ...
        self._snapshots[rev] = {
            str(p): p.read_text(encoding="utf-8", errors="replace") if p.exists() else None
            for p in paths
        }
        return rev
...
def _safe_path(path: str, sandbox_root: str) -> Path:
...
def _file_read(path: Path) -> str:
    return path.read_text(encoding="utf-8", errors="replace") if path.exists() else ""


def _compute_content(edit: Edit, original: str) -> dict:
    if edit.full_content is not None:
        return {"content": edit.full_content, "conflicts": [], "rejects": []}
    if edit.blocks:
        blocks = [
            OskillEditBlock(search=block.search, replace=block.replace) for block in edit.blocks
        ]
        result = _apply_edit_block(original, blocks=blocks)
        return {
    ...
    if edit.unified_diff:
        result = _apply_unified_diff(original, diff=edit.unified_diff)
        return {
            "content": result.content,
            "conflicts": result.conflicts,
            "rejects": result.rejects,
        }
    return {"content": None, "conflicts": ["edit has no content"], "rejects": []}


def _validate_edit(content: str, *, path: str, enabled: bool) -> dict:
    if not enabled or Path(path).suffix != ".py":
...
def _file_write(path: Path, *, content: str) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(content, encoding="utf-8")


def _write_trail(trail: list[dict], output_dir: Path, run_id: str) -> Path:
    path = output_dir / f"decision_trail_{run_id}.json"
    path.write_text(json.dumps(trail, ensure_ascii=False, indent=2), encoding="utf-8")
    return path
...
def _build_result(
    ...
    return {
        ...
        "decision_trail": {
            "path": str(output_dir / f"decision_trail_{run_id}.json"),
            "steps": len(trail),
            "run_id": run_id,
        },
        "cost_usd": 0.0,
    ...
    *,
    ...
    try:
        ...
        for step_idx, (edit, path) in enumerate(resolved):
            ...
            if new_content is None or conflicts or rejects:
                ...
                _record_step(
                    trail,
                    step_no=step_no,
                    event="edit_failed",  # pragma: no cover
                    path=edit.path,
                    issue=issue,
                )
                ...
                if on_step:  # pragma: no cover
                    on_step(
                        {"event": "edit_failed", "path": edit.path, "issue": issue}
                    )  # pragma: no cover
            ...
            if not validation["ok"]:
                ...
                if on_step:
                    on_step(
                        {
                            "event": "validation_failed",  # pragma: no cover
                            "path": edit.path,
                            "errors": validation["errors"],
                        }
                    )
        ...
        if need_rollback:
            ...
            _record_step(
                trail, step_no=len(trail) + 1, event="rollback_complete", restored=restored
            )
            if on_step:
        ...
        _record_step(
            trail, step_no=len(trail) + 1, event="unexpected_error", error=error
        )  # pragma: no cover
        # 尝试回滚（最大努力）
