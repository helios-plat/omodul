...
from oprim import dir_archive_to_targz
...
def _stage_archive(
    ...
    record_step(
        ...
        on_step=on_step,
        layer="oprim",
        callable_name="dir_archive_to_targz",
        inputs_summary={"source": path},
        outputs_summary={"status": "archived"},
...
def _stage_upload(
    ...
    record_step(
        ...
        on_step=on_step,
        layer="oprim",
        callable_name="s3_upload_file",
        inputs_summary={"target": url},
        outputs_summary={"status": "uploaded"},
        started_at=step_start,
    )
    return {"url": url}
...
def _stage_verify(
    ...
    record_step(
        ...
        on_step=on_step,
        layer="oprim",
        callable_name="s3_object_metadata",
        inputs_summary={"url": upload_info["url"]},
        outputs_summary={"status": "verified"},
        started_at=step_start,
    )


def compute_fingerprint_for_backup_app_data(
    config: BackupAppDataConfig, input_data: BackupAppDataInput
) -> str:
