...
class CindyMcpServer:
    ...
    def list_tools(self, category: str | None = None) -> dict[str, Any]:
        """Progressive discovery from categories to tool details."""
        if category is None:
            cats = {}
            for cat, names in self._categories.items():
                cats[cat] = {
                    "count": len(names),
                    "description": f"{cat} tools ({len(names)} available)",
                }
        ...
        if category not in self._categories:
            return {
                ...
                "available_categories": sorted(self._categories),
            }
...
def build_memory_mcp_server(memory_store: Any | None = None) -> CindyMcpServer:
    ...
    if store is not None:
        srv.register(
            ...
            lambda id: store.read(id) if hasattr(store, "read") else None,
            "read",
            {"type": "object", "properties": {"id": {"type": "string"}}, "required": ["id"]},
        )
        srv.register(
            "memory_list",
            "List memory entries (optionally by type).",
            lambda type=None: store.list_all(type) if hasattr(store, "list_all") else [],
            "read",
            {"type": "object", "properties": {"type": {"type": "string"}}},
        )
        srv.register(
            "memory_write",
            "Write/update a memory entry.",
            lambda id, type, body, **fm: (
                str(store.write(id, type, body, **fm)) if hasattr(store, "write") else "unavailable"
            ),
            "write",
            {
                "type": "object",
        ...
        srv.register(
            ...
            lambda id: store.delete(id) if hasattr(store, "delete") else False,
            "maintain",
            {"type": "object", "properties": {"id": {"type": "string"}}, "required": ["id"]},
        )
        srv.register(
            ...
            lambda query: [
                d
                for d in (store.list_all() if hasattr(store, "list_all") else [])
    ...
    srv.register(
        "memory_consolidate",
        "Consolidate stale memory entries (mark fresh).",
        lambda: _consolidate(store),
        "maintain",
    )
    srv.register(
        "memory_review",
        "Review stale memory entries.",
        lambda: [
            d["frontmatter"] for d in (store.list_stale() if hasattr(store, "list_stale") else [])
        ],
        "maintain",
    )
    return srv
...
def build_scheduler_mcp_server(scheduler: Any | None = None) -> CindyMcpServer:
    ...
    if sched is not None:
        ...
        srv.register(
            ...
            lambda id, name, prompt="", cron="", interval_ms=0: {
                "id": sched.create(id, name, prompt, cron, interval_ms).id
            },
            "write",
            {
                "type": "object",
        ...
        srv.register(
            ...
            lambda id, enabled=True: {"ok": sched.update(id, enabled=enabled) is not None},
            "write",
            {
                "type": "object",
