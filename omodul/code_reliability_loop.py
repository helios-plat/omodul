...
class FailureSignature:
    ...
    def from_test_result(cls, tr: TestResult, task_id: str = "") -> FailureSignature:
        if tr.metadata.get("timeout") or "timeout" in (tr.stderr or "").lower():
            ...
            kind, summary = (
                ...
                (
                    ...
                    if tr.failed_nodeids
                    else "测试失败 (无 nodeid 明细)"
                ),
            )
        else:
        ...
        return cls(kind=kind, summary=summary, fingerprint=fingerprint, evidence=evidence)
...
def run_code_reliability_loop(
    ...
    *,
    ...
    if task.spec_quality < SPEC_QUALITY_THRESHOLD:
        msg = (
            f"规格质量评分 {task.spec_quality:.2f} < {SPEC_QUALITY_THRESHOLD}: "
            "请补充可验证的测试/验收条件后重试"
        )
    ...
    # ── 1. 初始生成 ─────────────────────────────────────────────────
    parent: PatchArtifact | None = None
