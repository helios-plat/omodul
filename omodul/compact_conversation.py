...
async def compact_conversation(
    ...
    *,
    ...
    try:
        ...
        history = "\n".join(f"[{m.get('role', '?')}]: {_msg_text(m)[:400]}" for m in middle)
            ...
            [
                {
                    ...
                    "content": (
                        ...
                        f"preserving key technical decisions:\n\n{history[:6000]}"
                    ),
                }
            ],
        ...
        trail.record(
            event="compact_done", before=len(input_data.messages), after=len(compacted_messages)
        )
        if on_step:
