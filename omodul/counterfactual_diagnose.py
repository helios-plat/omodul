...
class CounterfactualDiagnosisReport:
    ...
    def summary(self) -> str:
        if not self.reports:
            return "无候选节点"
        top = self.reports[0]
        return (f"本次故障最可能被 '{top.node}' 压制: L3 压降 {top.l3_delta:.3f} "
    ...
    *,
    ...
    if candidate_nodes is None:
        candidate_nodes = [
            ...
            if n != failure_node and nx_has_path(dag, n, failure_node)
        ]
    ...
    factual_p = 1.0 if str(factual_evidence.get(failure_node, "")).lower() in (
        "fault", "1", "true", "fail") else 0.0
    ...
    for node in candidates:
        ...
        l3 = scm.l3_p_fault([node], failure_node, u_posterior)
        l2_delta = factual_p - l2
        l3_delta = factual_p - l3
        reports.append(CounterfactualReport(
            node=node,
