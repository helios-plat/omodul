...
def cross_asset_opportunity_ranking(
    ...
    *,
    ...
    try:
        ...
        norm_result = cross_asset_score_normalization(
            asset_scores=scores, asset_histories=histories
        )
        record_step(
            trail_steps=trail_steps,
            on_step=on_step,
            layer="oskill",
            callable_name="cross_asset_score_normalization",
            inputs_summary={"assets": list(scores.keys())},
            outputs_summary={"top": norm_result["top_opportunity"]},
        ...
        if len(assets_data) >= 2:
            gap = (assets_data[0].get("normalized_score") or 0) - (
                assets_data[1].get("normalized_score") or 0
            )
            insight = (
                f"{ranking[0]} leads by {gap:.1f} points over {ranking[1]}."
                if gap > 2
                else "No clear cross-asset opportunity today."
            )
        else:
            gap = 0
        ...
        record_step(
            ...
            on_step=on_step,
            layer="oprim",
            callable_name="insight_generation",
            inputs_summary={},
            outputs_summary={"insight": insight[:50]},
            started_at=step_start,
        )

        findings = {
            "normalized_ranking": assets_data,
            "top_pick": norm_result["top_opportunity"],
            "opportunity_gap": round(gap, 2),
        ...
        if "report" in config._enabled_pillars:
            ...
            lines = [
                f"# Cross-Asset Opportunity Ranking — {config.date}\n",
            ...
            for a in assets_data:
                lines.append(
                    f"- {a['asset']}: raw={a['raw_score']}, "
                    f"norm={a.get('normalized_score')}, pct={a.get('percentile_rank')}\n"
                )
