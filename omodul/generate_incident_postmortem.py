...
class GenerateIncidentPostmortemConfig(BaseConfig):
    ...
    _fingerprint_fields: ClassVar[set[str]] = {"incident_id", "time_window", "scope"}
...
def generate_incident_postmortem(
    ...
    *,
    ...
    try:
        ...
        findings = _stage_synthesize_postmortem(
            config, input_data, correlated, trail_steps, on_step
        )
...
def _stage_correlate_events(
    ...
    result = event_trail_correlate(target_event_id=target_id, all_events=input_data.event_trail)
    record_step(
        trail_steps=trail_steps,
        on_step=on_step,
        layer="oskill",
...
5. Impact Summary
"""
    response = llm(messages=[{"role": "user", "content": prompt}], max_tokens=4096)
...
def _custom_postmortem_section(findings: GenerateIncidentPostmortemFindings | None) -> str:
    ...
    timeline_lines = [
        f"- {e.get('timestamp')}: {e.get('event') or e.get('msg')}" for e in findings.timeline
    ]
    timeline = "\n".join(timeline_lines)
...
def compute_fingerprint_for_generate_incident_postmortem(
    config: GenerateIncidentPostmortemConfig, input_data: GenerateIncidentPostmortemInput
) -> str:
