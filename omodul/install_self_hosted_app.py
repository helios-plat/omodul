...
from typing import Callable, Literal, ClassVar, Any, Set, cast
...
class InstallSelfHostedAppConfig(BaseConfig):
    ...
    app_version: str
    instance_name: str = Field(..., description="实例名 (e.g. 'gitea-prod', 'gitea-dev'), 用于区分同一 app 的多实例")
    ...
    *,
    ...
    try:
        ...
        findings = InstallSelfHostedAppFindings(
            ...
            image_digest=str(image_info.get("Id", "")),
...
def _stage_pull_image(
    ...
    record_step(
        ...
        outputs_summary={"id": result.get("Id")}, started_at=step_start
    )
...
def _stage_start_container(
    ...
    # Get ID
    inspect = docker_container_inspect(container_id=config.instance_name, docker_host=input_data.docker_host)
    return {"id": str(inspect.get("Id", ""))}
...
def compute_fingerprint_for_install_self_hosted_app(config: InstallSelfHostedAppConfig, input_data: InstallSelfHostedAppInput) -> str:
