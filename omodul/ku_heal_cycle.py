...
def _remove_target(value: Any, target: str) -> Any:
    ...
    if isinstance(value, dict):
        return (
            None
            if str(value.get("ku_id", value.get("id", value.get("target", "")))) == target
            else value
        )
    return value
...
def _fix(issue: dict, units: dict[str, dict], backend: Any) -> tuple[str, bool, str]:
    ...
    if rule == "duplicate_fingerprint":
        ...
        matches = [(kid, item) for kid, item in units.items() if _fingerprint(item) == fp]
...
def ku_heal_cycle(
    config: KuHealCycleConfig,
    input_data: KuHealCycleInput,
    ...
    *,
    ...
    config = (
        config
        if isinstance(config, KuHealCycleConfig)
        else KuHealCycleConfig.model_validate(config)
    )
    input_data = (
        input_data
        if isinstance(input_data, KuHealCycleInput)
        else KuHealCycleInput.model_validate(input_data)
    )
    ...
    for issue in issues:
        ...
        trail.record(
            event="attempt_deterministic_fix", issue_id=issue["issue_id"], action=action, ok=ok
        )
        current = lint_knowledge_units(
            KuLintConfig(unverified_grade_ratio_warn=config.unverified_grade_ratio_warn),
            KuLintInput(
                knowledge_units=list(input_data.knowledge_units), backend=input_data.backend
            ),
        )
        remaining = [
            i for i in current if i["rule"] == issue["rule"] and i["ku_id"] == issue["ku_id"]
        ]
        ...
        if issue["rule"] == "duplicate_fingerprint":
            ...
            verified = (
                sum(
                    ...
                    if item.get("status") != "deprecated" and _fingerprint(item) == fp
                )
                <= 1
            )
        if verified and ok:
        ...
        actions.append(
            {
                ...
                "detail": detail if verified else f"verification failed: {remaining}",
            }
        )
    ...
    final_issues = lint_knowledge_units(
        KuLintConfig(unverified_grade_ratio_warn=config.unverified_grade_ratio_warn),
        KuLintInput(knowledge_units=list(input_data.knowledge_units), backend=input_data.backend),
    )
    for issue in remaining_open:
        if issue["rule"] == "duplicate_fingerprint" and not any(
            item["rule"] == issue["rule"] and item["ku_id"] == issue["ku_id"]
    ...
    return build_result(
        ...
        cost_usd=0.0,
        findings={"actions": actions, "issues_processed": len(issues)},
    )
