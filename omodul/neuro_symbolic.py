...
class NeuroSymbolicResult:
    ...
    repair: RepairPayload | None = None
    ...
    *,
                ...
                [
                    {
                        ...
                        "intent": d.intent,
                        "rendered": d.rendered,
                        "findings": [asdict(f) for f in d.findings if f.severity == "FAIL"],
                    }
                    for d in blocked
                ],
    ...
    if feas.status == "unknown":
        return NeuroSymbolicResult(
            ...
            repair=RepairPayload(
                "feasibility",
                f"求解器在 {feas_timeout_ms}ms 内无法判定(reason: {feas.reason_unknown})",
                [],
    ...
    # ---- 闸门 4: MaxSMT
    sol = optimize(c, timeout_ms=opt_timeout_ms, deterministic_tiebreak=deterministic_tiebreak)
    if sol.status != "sat":
        return NeuroSymbolicResult(
            False,
            plan_id,
            ...
            solution=sol,
            repair=RepairPayload(
                "solve", f"最优化阶段返回 {sol.status}", [], "放宽 timeout 或简化目标函数"
            ),
        )

    return NeuroSymbolicResult(
        True, plan_id, "done", ir=ir, diffs=diffs, feasibility=feas, solution=sol
    )
