...
from oprim._payments import RULES, PaymentResult, vcg
...
class OperatorDecision:
    ...
    @property
    def ok(self) -> bool:
        return not any(e.code in ("UNASSIGNED_TASK", "DEADLOCK_RISK") for e in self.escalations)


def run_operator_center(
    problem: Problem,
    *,
    mode: str = "auto",  # auto | one_to_one | capacity
    payment_rule: str | None = None,  # None=自家 Worker 不计价
    ...
    use_capacity = mode == "capacity" or (
        mode == "auto"
        and (
            any(w.max_tasks > 1 for w in problem.workers)
    ...
    # ③ 死锁安全的执行计划
    order = ResourceOrder(
        list(resource_ranking)
        if resource_ranking
        else sorted({r for t in problem.tasks for r in t.resources})
    )
    ...
    for wid, tids in alloc.by_worker().items():
        ...
        if bad:
            escalations.append(
                OperatorEscalation("ORDER_VIOLATION", f"{wid} 的申请序列存在逆序 {bad}", [wid])
            )
    ...
    if cycles:
        escalations.append(
            OperatorEscalation(
                "DEADLOCK_RISK", f"等待图中检测到 {len(cycles)} 个环", [">".join(c) for c in cycles]
            )
        )
