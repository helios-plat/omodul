...
async def parent_review_summary(
    config: ParentReviewConfig, input_data: ParentReviewInput, *, pool: Any
) -> dict:
    """家长端概览汇总。"""
    ...
    # 3. 提取数据 (Report)
    history = await get_student_history(
        pool=pool, student_id=input_data.student_id, limit=input_data.limit
    )
    error_dist = await get_error_distribution(pool=pool, student_id=input_data.student_id)

    trail.append(
        {"event": "data_fetched", "history_count": len(history), "dist_count": len(error_dist)}
    )
