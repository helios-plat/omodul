...
class ProposeActionPlanConfig(BaseConfig):
    ...
    _fingerprint_fields: ClassVar[set[str]] = {"root_cause_hash", "plugin_marketplace_version"}
...
def _stage_runbook_match(
    ...
    result = runbook_match(
        root_cause=input_data.root_cause, available_plugins=input_data.available_plugins
    )
    record_step(
        trail_steps=trail_steps,
        on_step=on_step,
...
def _stage_synthesize_plan(
    ...
    def local_search(q: str, c: str, k: int) -> list[RetrievedDoc]:
        return [
            RetrievedDoc(doc_id=str(i), content=str(r), score=1.0)
            for i, r in enumerate(input_data.historical_resolutions[:k])
        ]
...
def _stage_finalize_findings(
    match_result: RunbookMatchResult, synth_result: SynthesizedResult
) -> ProposeActionPlanFindings:
    ...
    if match_result.matched_plugin:
        ...
        approval: Literal["auto", "user_approval", "admin_approval"] = (
            "auto" if match_result.match_score > 0.9 and risk_val == "low" else "user_approval"
        )
        confidence = match_result.match_score
        risk: Literal["low", "medium", "high", "critical"] = (
            risk_val if risk_val in ("low", "medium", "high", "critical") else "medium"
        )
    else:
...
def compute_fingerprint_for_propose_action_plan(
    config: ProposeActionPlanConfig, input_data: ProposeActionPlanInput
) -> str:
