    ...
    try:
        ...
        for layer, name, fn in stages:
            ...
            record_step(
                ...
                callable_name=name,
                inputs_summary={},
                outputs_summary={"keys": list(result.keys())[:3]},
...
def cross_timeframe_analysis(
    config: CrossTimeframeConfig, input_data: dict, output_dir: Path, *, on_step=None
) -> dict:
...
def pack_promotion_workflow(
    config: PackPromotionConfig, input_data: dict, output_dir: Path, *, on_step=None
) -> dict:
    ...
    return _generic_workflow(
        ...
        stages=[
            (
                ...
                "pack_evaluation",
                lambda d: {
                    "promote": d.get("new_score", 0) > d.get("baseline", 0),
                    "improvement": 0.1,
                },
            ),
        ],
...
def regime_conditional_analysis(
    config: RegimeConditionalConfig, input_data: dict, output_dir: Path, *, on_step=None
) -> dict:
    ...
    return _generic_workflow(
        ...
        stages=[
            (
                ...
                lambda d: {"regime": d.get("regime", "unknown"), "ic_adjusted": True},
            ),
        ],
    )


def cross_sectional_ranking_workflow(
    config: CrossSectionalConfig, input_data: dict, output_dir: Path, *, on_step=None
) -> dict:
    ...
    return _generic_workflow(
        ...
        stages=[
            (
                ...
                lambda d: {
                    "ranks": {
                        a: i / max(len(config.assets) - 1, 1) for i, a in enumerate(config.assets)
                    }
                },
            ),
        ],
...
def walk_forward_validation(
    config: WalkForwardConfig, input_data: dict, output_dir: Path, *, on_step=None
) -> dict:
...
def strategy_capacity_assessment(
    config: CapacityConfig, input_data: dict, output_dir: Path, *, on_step=None
) -> dict:
    ...
    return _generic_workflow(
        ...
        stages=[
            (
                "oskill",
                "capacity_estimation",
                lambda d: {"capacity_usd": 5_000_000, "impact_at_capacity": 0.002},
            ),
        ],
    )


def signal_failure_postmortem(
    config: FailurePostmortemConfig, input_data: dict, output_dir: Path, *, on_step=None
) -> dict:
    ...
    return _generic_workflow(
        ...
        stages=[
            (
                "oskill",
                "failure_audit",
                lambda d: {"root_cause": d.get("cause", "unknown"), "preventable": True},
            ),
        ],
    )


def decision_log_correlation(
    config: DecisionLogConfig, input_data: dict, output_dir: Path, *, on_step=None
) -> dict:
    ...
    return _generic_workflow(
        ...
        stages=[
            (
                "oskill",
                "log_analysis",
                lambda d: {"decisions_analyzed": len(d.get("decisions", [])), "win_rate": 0.6},
            ),
        ],
    )
...
def counterfactual_analysis(
    config: CounterfactualConfig, input_data: dict, output_dir: Path, *, on_step=None
) -> dict:
    ...
    return _generic_workflow(
        ...
        stages=[
            (
                ...
                lambda d: {"scenarios": d.get("perturbations", {}), "impact": "moderate"},
            ),
        ],
    )
...
def insight_generation(
    config: InsightConfig, input_data: dict, output_dir: Path, *, on_step=None
) -> dict:
    ...
    return _generic_workflow(
        ...
        stages=[
            ...
            (
                "external",
                "llm_insight",
                lambda d: {"insight": f"Market insight for {config.symbol}", "confidence": 0.7},
            ),
        ],
    )


def cold_start_briefing(
    config: ColdStartConfig, input_data: dict, output_dir: Path, *, on_step=None
) -> dict:
    ...
    return _generic_workflow(
        ...
        stages=[
            (
                "oskill",
                "briefing_compile",
                lambda d: {"briefing": "Welcome briefing", "sections": 3},
            ),
        ],
    )


def multi_source_data_collection(
    config: MultiSourceConfig, input_data: dict, output_dir: Path, *, on_step=None
) -> dict:
    ...
    return _generic_workflow(
        ...
        stages=[
            (
                "oprim_batch",
                "collect_sources",
                lambda d: {"sources_collected": len(config.sources), "success": True},
            ),
        ],
    )


def ohlcv_merge_workflow(
    config: OhlcvMergeConfig, input_data: dict, output_dir: Path, *, on_step=None
) -> dict:
    ...
    return _generic_workflow(
        ...
        stages=[
            (
                "oprim_batch",
                "fetch_exchanges",
                lambda d: {"exchanges_fetched": len(config.exchanges)},
            ),
