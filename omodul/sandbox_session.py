...
class SandboxSession:
    ...
    def put_file(self, path: str, content: str | bytes) -> dict[str, Any]:
        ...
        return sandbox_put_file(self.sandbox_id, path, content)

    def get_file(self, path: str) -> dict[str, Any]:
        if not self.ok:
            return {"ok": False, "error": self.error or "sandbox not open"}
        return sandbox_get_file(self.sandbox_id, path)

    def list(self, path: str = ".") -> dict[str, Any]:
        if not self.ok:
            return {"ok": False, "error": self.error or "sandbox not open"}
        return sandbox_list(self.sandbox_id, path)

    def apply_patch(self, patch: str) -> dict[str, Any]:
        if not self.ok:
            return {"ok": False, "error": self.error or "sandbox not open"}
        return sandbox_apply_patch(self.sandbox_id, patch)

    def close(self) -> None:
        if self._closed:
            return
        if self.sandbox_id:
            sandbox_destroy(self.sandbox_id)
        broker = get_broker()
        if self._slot:
            broker.release_slot(self._slot)
            self._slot = ""
...
def sandbox_session(purpose: str = "chat_verify", **overrides: Any) -> SandboxSession:
    ...
    policy = isolation_policy(purpose)
    ...
    if not broker.acquire_slot(str(spec["isolation"]), timeout=60.0):
        session.error = f"sandbox slot {spec['isolation']!r} busy"
        session.close()
        return session
...
def eval_in_sandbox(
    *,
    ...
    with sandbox_scope(purpose) as session:
        if not session.ok:
            return {
                "ok": False,
                "passed": False,
