...
class TriageSignalConfig(BaseConfig):
    ...
    _fingerprint_fields: ClassVar[set[str]] = {"signal_hash", "context_hash"}
...
def _stage_llm_triage(
    ...
    prompt = f"""You are a signal triage assistant. Given a signal and context,
classify priority and route it.
...
"""
    response = llm(messages=[{"role": "user", "content": prompt}], max_tokens=config.max_tokens)
...
def compute_fingerprint_for_triage_signal(
    config: TriageSignalConfig, input_data: TriageSignalInput
) -> str:
