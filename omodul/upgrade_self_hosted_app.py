...
class UpgradeSelfHostedAppConfig(BaseConfig):
    ...
    _fingerprint_fields: ClassVar[set[str]] = {"instance_name", "current_version", "target_version"}
    instance_name: str
    current_version: str
    target_version: str
...
def _stage_pull_new_image(
    ...
    record_step(
        ...
        inputs_summary={"image": input_data.new_image},
        outputs_summary={"status": "pulled"},
...
def _stage_stop_old_container(
    ...
    record_step(
        ...
        on_step=on_step,
        layer="oprim",
        callable_name="docker_container_stop",
        inputs_summary={"id": input_data.container_id},
        outputs_summary={"status": "stopped"},
        started_at=step_start,
    )
...
def _stage_start_new_container(
    ...
    record_step(
        ...
        inputs_summary={"id": input_data.container_id},
        outputs_summary={"status": "started"},
        started_at=step_start,
    )
...
def _stage_verify_health(
    ...
    try:
        record_step(
            ...
            on_step=on_step,
            layer="oprim",
            callable_name="verify_health",
            inputs_summary={},
            outputs_summary={"healthy": True},
            started_at=step_start,
        )
        return True
    except Exception:
...
def _stage_handle_failure(
    ...
    record_step(
        ...
        on_step=on_step,
        layer="oprim",
        callable_name="rollback",
        inputs_summary={},
        outputs_summary={"status": "rolled back"},
        started_at=step_start,
    )


def compute_fingerprint_for_upgrade_self_hosted_app(
    config: UpgradeSelfHostedAppConfig, input_data: UpgradeSelfHostedAppInput
) -> str:
