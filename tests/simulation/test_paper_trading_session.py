...
def _ohlcv(symbol="AAPL", open_=150.0, close=152.0, high=155.0, low=149.0, prev_close=150.0):
    return {
        symbol: {"open": open_, "close": close, "high": high, "low": low, "prev_close": prev_close}
    }
...
class TestPaperTradingSession:
    ...
    def test_sell_after_buy_updates_pnl(self):
        ...
        r1 = paper_trading_session(
            acct, buy, _ohlcv(open_=100.0, close=100.0, prev_close=100.0), _rules(), ts
        )
    ...
    def test_t_plus_1_blocked(self):
        ...
        acct["positions"] = [
            {"symbol": "AAPL", "qty": 10, "entry_price": 150.0, "entry_date": date(2026, 1, 5)}
        ]
    ...
    def test_limit_up_blocks_buy(self):
        ...
        result = paper_trading_session(
            _account(), orders, ohlcv, _rules(0.10), datetime(2026, 1, 5)
        )
    ...
    def test_limit_down_blocks_sell(self):
        ...
        acct = _account()
        acct["positions"] = [
            {"symbol": "AAPL", "qty": 10, "entry_price": 100.0, "entry_date": date(2026, 1, 4)}
        ]
        # close 10.1% below prev_close → limit down
        ohlcv = {
            "AAPL": {"open": 89.0, "close": 89.0, "high": 90.0, "low": 88.0, "prev_close": 100.0}
        }
        sell = [{"order_id": "s1", "symbol": "AAPL", "side": "sell", "quantity": 10}]
    ...
    def test_unknown_order_type_uses_open(self):
        ...
        orders = [
            {"order_id": "o1", "symbol": "AAPL", "side": "buy", "quantity": 5, "order_type": "vwap"}
        ]
        result = paper_trading_session(
    ...
    def test_partial_sell_updates_position(self):
        ...
        acct = _account()
        acct["positions"] = [
            {"symbol": "AAPL", "qty": 20, "entry_price": 100.0, "entry_date": date(2026, 1, 4)}
        ]
        sell = [
            {
                "order_id": "s1",
        ...
        result = paper_trading_session(
            acct, sell, _ohlcv(open_=100.0, prev_close=99.0), _rules(), ts
        )
        assert len(result["executed_trades"]) == 1
