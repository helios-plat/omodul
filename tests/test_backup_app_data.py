...
def test_backup_fingerprint_stability():
    config = BackupAppDataConfig(
        instance_name="g-prod", backup_target="s3://b", scope="full", time_window="manual"
    )
...
def test_backup_fingerprint_changes_whitelist():
    config1 = BackupAppDataConfig(
        instance_name="g-prod", backup_target="s3://b", scope="full", time_window="w1"
    )
    config2 = BackupAppDataConfig(
        instance_name="g-prod", backup_target="s3://b", scope="full", time_window="w2"
    )
    input_data = BackupAppDataInput(container_id="c1", volumes_to_backup=["/v"], config_paths=[])
    from omodul.backup_app_data import compute_fingerprint_for_backup_app_data

    assert compute_fingerprint_for_backup_app_data(
        config1, input_data
    ) != compute_fingerprint_for_backup_app_data(config2, input_data)
...
def test_backup_with_config_paths(tmp_path):
    ...
    input_data = BackupAppDataInput(
        container_id="c1", volumes_to_backup=[], config_paths=["/etc/app.conf"]
    )
    with patch("omodul.backup_app_data.dir_archive_to_targz"):
        with patch("omodul.backup_app_data.s3_upload_file"):
            with patch("omodul.backup_app_data.s3_object_metadata"):
