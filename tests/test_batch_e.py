...
class TestApplyChangeset:
    ...
    def test_syntax_error_rolls_back(self, tmp_path):
        ...
        r = apply_changeset(
            ChangesetConfig(), ChangesetInput(edits=edits, versionstore=vstore), tmp_path / "out"
        )
    ...
    def test_sandbox_violation_fails(self, tmp_path):
        ...
        r = apply_changeset(
            ChangesetConfig(sandbox_root=str(sandbox)),
            ChangesetInput(edits=edits),
            tmp_path / "out",
        )
    ...
    def test_multi_file_second_fails_rollback(self, tmp_path):
        ...
        r = apply_changeset(
            ChangesetConfig(), ChangesetInput(edits=edits, versionstore=vs), tmp_path / "out"
        )
...
class TestRunSubagent:
    ...
    def test_completes(self, tmp_path):
        cfg = SubagentConfig()
        inp = SubagentInput(
            task="do task", subagent_def=self._defn(), caller=make_caller("task done")
        )
    ...
    def test_summary_contains_result(self, tmp_path):
        r = run(
            run_subagent(
                SubagentConfig(),
                SubagentInput(
                    task="test", subagent_def=self._defn(), caller=make_caller("result text")
                ),
                tmp_path,
            )
        )
    ...
    def test_depth_exceeded(self, tmp_path):
        from omodul.run_subagent import RECURSION_DEPTH_LIMIT, _current_depth
    ...
    def test_completes(self, tmp_path):
        r = run(
            initialize_project(
                InitProjectConfig(),
                InitProjectInput(
                    root_path=str(tmp_path), caller=make_caller("# AGENTS.md\nProject desc.")
                ),
                tmp_path / "out",
            )
        )
    ...
    def test_conventional_style(self, tmp_path):
        r = run(
            generate_commit_message(
                ...
                GenerateCommitInput(
                    repo_path=str(tmp_path), caller=make_caller("feat: x"), diff_text="+x=1"
                ),
                tmp_path / "out",
            )
        )
    ...
    def test_cost_tracked(self, tmp_path):
        r = run(
            generate_commit_message(
                GenerateCommitConfig(),
                GenerateCommitInput(
                    repo_path=str(tmp_path), caller=make_caller("msg"), diff_text="+x=1"
                ),
                tmp_path / "out",
            )
        )
    ...
    def test_truncates_large_diff(self, tmp_path):
        ...
        r = run(
            generate_commit_message(
                ...
                GenerateCommitInput(
                    repo_path=str(tmp_path), caller=make_caller("msg"), diff_text=big_diff
                ),
                tmp_path / "out",
            )
        )
    ...
    def test_completes(self, tmp_path):
        ...
        r = run(
            generate_tests(
                GenerateTestsConfig(),
                GenerateTestsInput(
                    target_path=str(f), caller=make_caller("def test_add(): assert add(1,2)==3")
                ),
                tmp_path / "out",
            )
        )
    ...
    def test_custom_output_path(self, tmp_path):
        ...
        run(
            generate_tests(
                ...
                GenerateTestsInput(
                    target_path=str(f),
                    caller=make_caller("def test(): pass"),
    ...
    def test_completes(self, tmp_path):
        ...
        r = run(
            explain_codebase(
                ExplainCodebaseConfig(),
                ExplainCodebaseInput(
                    root_path=str(tmp_path), caller=make_caller("This is a project.")
                ),
                tmp_path / "out",
            )
        )
...
class TestSecurityAudit:
    def test_completes(self, tmp_path):
        f = tmp_path / "app.py"
        f.write_text("password='hardcoded123'\n")
        r = run(
            security_audit(
                SecurityAuditConfig(),
                SecurityAuditInput(
                    paths=[str(f)], caller=make_caller("Found: hardcoded password.")
                ),
                tmp_path / "out",
            )
        )
...
class TestRefactorTransaction:
    def test_completes(self, tmp_path):
        ...
        r = run(
            refactor_transaction(
                RefactorConfig(),
                RefactorInput(
                    instruction="rename foo to bar", paths=[str(f)], caller=make_caller(edits_json)
                ),
                tmp_path / "out",
            )
        )
    ...
    def test_file_modified(self, tmp_path):
        ...
        run(
            refactor_transaction(
                RefactorConfig(),
                RefactorInput(instruction="rename", paths=[str(f)], caller=make_caller(edits_json)),
                tmp_path / "out",
            )
        )
    ...
    def test_report_written(self, tmp_path):
        ...
        r = run(
            refactor_transaction(
                RefactorConfig(),
                RefactorInput(instruction="refactor", paths=[str(f)], caller=make_caller("[]")),
                tmp_path / "out",
            )
        )
    ...
    def test_invalid_json_edits(self, tmp_path):
        ...
        r = run(
            refactor_transaction(
                RefactorConfig(),
                RefactorInput(
                    instruction="refactor", paths=[str(f)], caller=make_caller("not json at all")
                ),
                tmp_path / "out",
            )
        )
    ...
    def test_error_fails(self, tmp_path):
        ...
        r = run(
            refactor_transaction(
                RefactorConfig(),
                RefactorInput(instruction="refactor", paths=[], caller=bad),
                tmp_path / "out",
            )
        )
        assert r["status"] == "failed"
    ...
    def test_fingerprint(self, tmp_path):
        ...
        r = run(
            refactor_transaction(
                RefactorConfig(),
                RefactorInput(instruction="rename", paths=[str(f)], caller=make_caller("[]")),
                tmp_path / "out",
            )
        )
        assert r["fingerprint"]
...
class TestCreateCheckpoint:
    ...
    def test_checkpoint_id_unique(self, tmp_path):
        r1 = run(
            create_checkpoint(
                CreateCheckpointConfig(), CreateCheckpointInput(messages=MSGS), tmp_path
            )
        )
        r2 = run(
            create_checkpoint(
    ...
    def test_store_error_fails(self, tmp_path):
        ...
        r = run(
            create_checkpoint(
                CreateCheckpointConfig(),
                CreateCheckpointInput(messages=MSGS, store=store),
                tmp_path,
            )
        )
        assert r["status"] == "failed"
...
class TestRewindToCheckpoint:
    ...
    def test_metadata_restored(self, tmp_path):
        cr = run(
            create_checkpoint(
                CreateCheckpointConfig(),
                CreateCheckpointInput(messages=MSGS, metadata={"key": "value"}),
                tmp_path,
            )
        )
        r = run(
        ...
        r = run(
            migrate_dependency(
                ...
                MigrateDependencyInput(
                    ...
                    caller=make_caller("Migration guide."),
                ),
                tmp_path / "out",
            )
        )
    ...
    def test_error_fails(self, tmp_path):
        ...
        r = run(
            migrate_dependency(
                MigrateDependencyConfig(),
                MigrateDependencyInput(
                    dependency="x", target_version="1.0", root_path=str(tmp_path), caller=bad
                ),
                tmp_path / "out",
            )
        )
    ...
    def test_completes(self, tmp_path):
        r = run(
            install_plugin(
                InstallPluginConfig(),
                InstallPluginInput(
                    plugin_bundle=self.BUNDLE, install_dir=str(tmp_path / "plugins")
                ),
                tmp_path / "out",
            )
        )
    ...
    def test_missing_name_fails(self, tmp_path):
        r = run(
            install_plugin(
                InstallPluginConfig(),
                InstallPluginInput(
                    plugin_bundle={"version": "1.0"}, install_dir=str(tmp_path / "plugins")
                ),
                tmp_path / "out",
            )
        )
