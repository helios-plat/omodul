...
class TestOhlcvBackfill:
    ...
    async def test_with_pool_writes_all_bars(self):
        ...
        mock_pool = MagicMock()
        with (
            patch("oprim.ohlcv_fetch.ohlcv_fetch", new_callable=AsyncMock) as m,
...
class TestAuditRecord:
    ...
    def _config(self, priv_b64=""):
        ...
        return AuditRecordConfig(
            event_type="trade_executed", actor_id="user_001", private_key_b64=priv_b64
        )
...
class TestLlmAlphaMine:
    ...
    def test_fingerprint_changes_with_context(self):
        from omodul.llm_alpha_mine import LlmAlphaMineConfig, llm_alpha_mine
        ...
        r1 = llm_alpha_mine(
            data, config=cfg1, llm_caller=self._caller(), strategy_fn=self._strategy
        )
        r2 = llm_alpha_mine(
            data, config=cfg2, llm_caller=self._caller(), strategy_fn=self._strategy
        )
        assert r1["fingerprint"] != r2["fingerprint"]
    ...
    def test_factor_hypothesis_in_debate_prompt(self):
        ...
        llm_alpha_mine(
            list(range(30)), config=self._config(), llm_caller=capturing, strategy_fn=self._strategy
        )
