...
from omodul.configure_domain_for_app import (
    ConfigureDomainConfig,
    ...
    with patch("omodul.configure_domain_for_app.dns_resolve", return_value=["1.2.3.4"]):
        with patch("omodul.configure_domain_for_app.caddy_admin_reload"):
            with patch("omodul.configure_domain_for_app.caddy_certificates_status") as mock_cert:
                ...
                assert result["findings"].dns_resolved
                assert result["findings"].https_certificate_obtained
...
def test_domain_https_not_obtained(tmp_path):
    ...
    with patch("omodul.configure_domain_for_app.dns_resolve", return_value=["1.2.3.4"]):
        with patch("omodul.configure_domain_for_app.caddy_admin_reload"):
            with patch(
                "omodul.configure_domain_for_app.caddy_certificates_status", return_value=[]
            ):
                result = configure_domain_for_app(config, input_data, tmp_path)
                assert result["status"] == "completed"
                assert not result["findings"].https_certificate_obtained
...
def test_domain_fingerprint_changes_whitelist():
    ...
    assert compute_fingerprint_for_configure_domain_for_app(
        config1, input_data
    ) != compute_fingerprint_for_configure_domain_for_app(config2, input_data)
