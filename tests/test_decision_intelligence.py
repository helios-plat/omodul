...
def test_decision_ledger_record_link_trace(tmp_path):
    ...
    decision_ledger(
        cfg,
        DecisionLedgerInput(
            action="link", src_id=ids[0], dst_id=ids[1], relationship_type="CAUSED"
        ),
        tmp_path,
    )
    decision_ledger(
        cfg,
        DecisionLedgerInput(
            action="link", src_id=ids[0], dst_id=ids[2], relationship_type="INFLUENCED"
        ),
        tmp_path,
    )
    # trace
    ...
    q = decision_ledger(
        cfg,
        DecisionLedgerInput(
            action="query_similar", query_text="个人贷款 8.5w 收入 31% DTI", max_results=3
        ),
        tmp_path,
    )
    assert q["findings"]["precedents"][0]["decision_id"] == ids[0]
    # 策略门: confidence 阈值
    rules = decision_ledger(
        cfg,
        DecisionLedgerInput(action="rules", rules=[{"rule": "confidence_min", "min": 0.95}]),
        tmp_path,
    )
...
def test_decision_ledger_validation(tmp_path):
    ...
    try:
        decision_ledger(
            cfg, DecisionLedgerInput(action="record", scenario="", outcome="x"), tmp_path
        )
    ...
    try:
        decision_ledger(
            cfg,
            DecisionLedgerInput(action="link", src_id="a", dst_id="b", relationship_type="BOGUS"),
            tmp_path,
        )
...
def test_provenance_w3c_json_and_turtle(tmp_path):
    ...
    j = provenance_w3c(
        cfg, ProvenanceW3cInput(entities=entities, relations=relations, format="json"), tmp_path
    )
    ...
    provenance_w3c(
        cfg, ProvenanceW3cInput(entities=entities, relations=relations, format="turtle"), tmp_path
    )
