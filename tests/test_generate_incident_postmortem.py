...
from omodul.generate_incident_postmortem import (
...
def test_postmortem_fingerprint_stability():
    ...
    input_data = GenerateIncidentPostmortemInput(
        incident_id="I1", event_trail=[], involved_services=[], resolutions_applied=[]
    )
    from omodul.generate_incident_postmortem import (
...
def test_postmortem_completed_normal_path(tmp_path):
    ...
    with patch("obase.ProviderRegistry.get") as mock_reg_get:
        ...
        mock_provider.create_caller.return_value = MagicMock(
            return_value={"content": "Postmortem Content"}
        )
        mock_reg_get.return_value = mock_provider

        with patch("omodul.generate_incident_postmortem.event_trail_correlate") as mock_corr:
            mock_corr.return_value = CorrelatedEvents(
                target_event_id="E1", causally_related=[], time_window_correlated=[], confidence=1.0
            )
...
def test_postmortem_timeline_only_scope(tmp_path):
    config = GenerateIncidentPostmortemConfig(
        incident_id="I1", time_window="2026-05-24", scope="timeline_only"
    )
    input_data = GenerateIncidentPostmortemInput(
        incident_id="I1",
        event_trail=[{"event": "e1"}],
        involved_services=[],
        resolutions_applied=[],
    )
...
def test_postmortem_decision_trail(tmp_path):
    config = GenerateIncidentPostmortemConfig(incident_id="I1", time_window="2026-05-24")
    input_data = GenerateIncidentPostmortemInput(
        incident_id="I1", event_trail=[{"id": "1"}], involved_services=[], resolutions_applied=[]
    )
    ...
    with patch("obase.ProviderRegistry.get") as mock_reg_get:
        ...
        with patch("omodul.generate_incident_postmortem.event_trail_correlate") as mock_corr:
            mock_corr.return_value = CorrelatedEvents(
                target_event_id="1", causally_related=[], time_window_correlated=[], confidence=1.0
            )

            result = generate_incident_postmortem(config, input_data, tmp_path)
            assert any(
                s["callable"] == "llm_synthesize_postmortem"
                for s in result["decision_trail"]["steps"]
            )
...
def test_postmortem_fingerprint_changes_whitelist():
    ...
    assert compute_fingerprint_for_generate_incident_postmortem(
        config1, input_data
    ) != compute_fingerprint_for_generate_incident_postmortem(config2, input_data)
...
def test_postmortem_invariant_outside_whitelist():
    config1 = GenerateIncidentPostmortemConfig(
        incident_id="I1", time_window="2026-05-24", budget_usd=5.0
    )
    config2 = GenerateIncidentPostmortemConfig(
        incident_id="I1", time_window="2026-05-24", budget_usd=10.0
    )
    input_data = GenerateIncidentPostmortemInput(
        incident_id="I1", event_trail=[], involved_services=[], resolutions_applied=[]
    )
