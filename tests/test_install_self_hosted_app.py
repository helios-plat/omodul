...
from omodul.install_self_hosted_app import (
    InstallSelfHostedAppConfig,
    InstallSelfHostedAppInput,
...
def test_install_fingerprint_stability():
    config = InstallSelfHostedAppConfig(
        app_slug="gitea", app_version="1.0", instance_name="g-prod", config_hash="h1"
    )
...
def test_install_completed_normal_path(tmp_path):
    config = InstallSelfHostedAppConfig(
        app_slug="gitea", app_version="1.0", instance_name="g-prod", config_hash="h1", domain="g.io"
    )
    input_data = InstallSelfHostedAppInput(app_config={"ports": {"80/tcp": 3000}})

    with patch(
        "omodul.install_self_hosted_app.docker_image_pull", return_value={"Id": "sha256:image"}
    ):
        with patch("omodul.install_self_hosted_app.docker_container_start", return_value={}):
            with patch(
                "omodul.install_self_hosted_app.docker_container_inspect",
                return_value={"Id": "c1", "State": {"Running": True}},
            ):
                with patch(
                    "omodul.install_self_hosted_app.http_health_probe",
                    return_value={"healthy": True},
                ):
                    with patch(
                        "omodul.install_self_hosted_app.caddy_admin_reload", return_value={}
                    ):
                        result = install_self_hosted_app(config, input_data, tmp_path)
                        assert result["status"] == "completed"
                        assert result["findings"].container_id == "c1"
                        assert result["findings"].https_active


def test_install_without_domain(tmp_path):
    config = InstallSelfHostedAppConfig(
        app_slug="gitea", app_version="1.0", instance_name="g-prod", config_hash="h1", domain=None
    )
    input_data = InstallSelfHostedAppInput(app_config={})
        ...
        with patch("omodul.install_self_hosted_app.docker_container_start", return_value={}):
            with patch(
                ...
                with patch(
                    ...
                    assert not result["findings"].https_active
...
def test_install_decision_trail(tmp_path):
    config = InstallSelfHostedAppConfig(
        app_slug="a", app_version="1", instance_name="n", config_hash="h"
    )
    input_data = InstallSelfHostedAppInput(app_config={})
...
def test_install_fingerprint_changes_whitelist():
    config1 = InstallSelfHostedAppConfig(
        app_slug="a", app_version="1", instance_name="n", config_hash="h1"
    )
    config2 = InstallSelfHostedAppConfig(
        app_slug="a", app_version="1", instance_name="n", config_hash="h2"
    )
    input_data = InstallSelfHostedAppInput(app_config={})
    from omodul.install_self_hosted_app import compute_fingerprint_for_install_self_hosted_app

    assert compute_fingerprint_for_install_self_hosted_app(
        config1, input_data
    ) != compute_fingerprint_for_install_self_hosted_app(config2, input_data)
...
def test_install_caddy_failure_still_completed_partial(tmp_path):
    config = InstallSelfHostedAppConfig(
        app_slug="a", app_version="1", instance_name="n", config_hash="h", domain="d.io"
    )
    input_data = InstallSelfHostedAppInput(app_config={})
