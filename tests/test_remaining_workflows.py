...
from omodul.remaining_workflows import (
    CapacityConfig,
    ColdStartConfig,
    ...
    cross_sectional_ranking_workflow,
    cross_timeframe_analysis,
    decision_log_correlation,
    insight_generation,
    multi_source_data_collection,
    ohlcv_merge_workflow,
    pack_promotion_workflow,
    regime_conditional_analysis,
    signal_failure_postmortem,
    strategy_capacity_assessment,
...
def test_pack_promotion(tmp):
    r = pack_promotion_workflow(
        PackPromotionConfig(pack_id="v2"), {"new_score": 80, "baseline": 60}, tmp
    )
    assert r["status"] == "completed"
...
def test_failure_postmortem(tmp):
    r = signal_failure_postmortem(
        FailurePostmortemConfig(signal_id="s-123"), {"cause": "data_stale"}, tmp
    )
...
def test_counterfactual(tmp):
    r = counterfactual_analysis(
        CounterfactualConfig(base_date="2024-01-01"), {"perturbations": {"vol": 0.5}}, tmp
    )
    assert r["status"] == "completed"
