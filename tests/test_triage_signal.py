...
from omodul.triage_signal import (
    TriageSignalConfig,
    TriageSignalInput,
    triage_signal,
)
...
def test_triage_signal_decision_trail_structure(tmp_path):
    ...
    with patch("obase.ProviderRegistry.get") as mock_reg_get:
        ...
        mock_caller = MagicMock(
            return_value={
                "content": (
                    '{"priority": "P2", "category": "user", '
                    '"should_escalate": false, "routing_hint": "none", '
                    '"confidence": 0.5, "reasoning_summary": "low"}'
                )
            }
        )
        mock_provider.create_caller.return_value = mock_caller
...
def test_triage_signal_report_content(tmp_path):
    ...
    with patch("obase.ProviderRegistry.get") as mock_reg_get:
        ...
        mock_caller = MagicMock(
            return_value={
                "content": (
                    '{"priority": "P1", "category": "db", '
                    '"should_escalate": true, "routing_hint": "db_team", '
                    '"confidence": 0.8, "reasoning_summary": "slow queries"}'
                )
            }
        )
        mock_provider.create_caller.return_value = mock_caller
...
def test_triage_signal_on_step_callback(tmp_path):
    ...
    with patch("obase.ProviderRegistry.get") as mock_reg_get:
        ...
        mock_caller = MagicMock(
            return_value={
                "content": (
                    '{"priority": "P3", "category": "info", '
                    '"should_escalate": false, "routing_hint": "log", '
                    '"confidence": 1.0, "reasoning_summary": "ok"}'
                )
            }
        )
        mock_provider.create_caller.return_value = mock_caller
...
def test_triage_signal_json_with_extra_text(tmp_path):
    ...
    with patch("obase.ProviderRegistry.get") as mock_reg_get:
        ...
        mock_caller.return_value = {
            "content": (
                'Here is the analysis: {"priority": "P0", "category": "infra", '
                '"should_escalate": true, "routing_hint": "rca", '
                '"confidence": 0.9, "reasoning_summary": "test"} '
                "Hope this helps!"
            )
