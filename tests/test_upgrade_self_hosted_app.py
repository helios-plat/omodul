...
from omodul.upgrade_self_hosted_app import (
    UpgradeSelfHostedAppConfig,
    UpgradeSelfHostedAppInput,
...
def test_upgrade_fingerprint_stability():
    config = UpgradeSelfHostedAppConfig(
        instance_name="g-prod", current_version="1.0", target_version="1.1"
    )
    ...
    with patch("omodul.upgrade_self_hosted_app.docker_image_pull"):
        with patch("omodul.upgrade_self_hosted_app.docker_container_stop"):
            with patch("omodul.upgrade_self_hosted_app.docker_container_start"):
                ...
                assert not result["findings"].rolled_back


def test_upgrade_rollback_on_health_failure(tmp_path):
    config = UpgradeSelfHostedAppConfig(
        instance_name="g-prod",
        current_version="1.0",
        target_version="1.1",
        ...
        with patch("omodul.upgrade_self_hosted_app.docker_container_stop"):
            with patch("omodul.upgrade_self_hosted_app.docker_container_start"):
                with patch("omodul.upgrade_self_hosted_app._stage_verify_health") as mock_verify:
                    ...
                    assert result["findings"].rolled_back
...
def test_upgrade_fingerprint_changes_whitelist():
    config1 = UpgradeSelfHostedAppConfig(
        instance_name="g-prod", current_version="1.0", target_version="1.1"
    )
    config2 = UpgradeSelfHostedAppConfig(
        instance_name="g-prod", current_version="1.0", target_version="1.2"
    )
    input_data = UpgradeSelfHostedAppInput(container_id="c1", new_image="g:1.1")
    from omodul.upgrade_self_hosted_app import compute_fingerprint_for_upgrade_self_hosted_app

    assert compute_fingerprint_for_upgrade_self_hosted_app(
        config1, input_data
    ) != compute_fingerprint_for_upgrade_self_hosted_app(config2, input_data)


def test_upgrade_invariant_outside_whitelist():
    config1 = UpgradeSelfHostedAppConfig(
        instance_name="g-prod", current_version="1.0", target_version="1.1", budget_usd=5.0
    )
    config2 = UpgradeSelfHostedAppConfig(
        instance_name="g-prod", current_version="1.0", target_version="1.1", budget_usd=10.0
    )
    input_data = UpgradeSelfHostedAppInput(container_id="c1", new_image="g:1.1")
    from omodul.upgrade_self_hosted_app import compute_fingerprint_for_upgrade_self_hosted_app
...
def test_upgrade_rollback_logic_step(tmp_path):
    ...
    with patch("omodul.upgrade_self_hosted_app.docker_image_pull"):
        with patch("omodul.upgrade_self_hosted_app.docker_container_stop"):
            with patch("omodul.upgrade_self_hosted_app.docker_container_start"):
                with patch(
                    "omodul.upgrade_self_hosted_app._stage_verify_health", return_value=False
                ):
                    result = upgrade_self_hosted_app(config, input_data, tmp_path)
                    assert any(
                        s["callable"] == "rollback" for s in result["decision_trail"]["steps"]
                    )
